...
func (it *prefixIterator) NodeBlob() []byte {
	if it.hasPrefix() {
		return it.nodeIterator.NodeBlob()
	}
	return nil
}
...
func (it *prefixIterator) AddResolver(reader ethdb.KeyValueReader) {
	if it.hasPrefix() {
		it.nodeIterator.AddResolver(reader)
	}
}
